...
private applyBossGimmickEffect(boss: MonsterData, g: BossGimmick): void {
  ...
  switch (g.effect) {
    ...
    case 'REVIVE':
      // ランタイム現在HPを50%で復活、防壁リセット（stats.hp は最大HPとして維持）
...
this.checkBossGimmicks(target as unknown as MonsterData & { gimmicks?: BossGimmick[] }, hpChange.prevHpPct, hpChange.newHpPct);
