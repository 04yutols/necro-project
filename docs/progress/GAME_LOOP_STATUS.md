...
> 確認日: 2026-05-17  
> `npx tsc --noEmit` → OK ✓  
> `npm test --runInBand` → 17 suites / 92 tests / all passed ✓
...
| ドロップ報酬（武器 / 残滓 / 素材）| ✅ | RewardService.test で確認済み |
| リザルト画面 | ✅ | 獲得アイテム表示・onFinish で MAP 帰還 |
...
| DB連携（processStageResultAction）| ✅ | 未ログイン時はスキップ・ローカルに保存 |
...
### 1. バトル内アイテムがハードコード
**ファイル**: `src/components/battle/BattleCanvas.tsx` L149-151  
**問題**: `ITEMS` 定数が固定値（冥界薬×3・エーテル×2）。`inventoryItems` ストアと未接続。  
消費してもインベントリから減らず、バトルで入手したアイテムも道具として使えない。

```typescript
// 現状
const ITEMS = [
  { id: 0, name: '冥界薬', count: 3, ... },
  { id: 1, name: 'エーテル', count: 2, ... },
];

// 本来あるべき姿
const items = useGameStore(s => s.inventoryItems).filter(i => i.type === 'CONSUMABLE');
```

---

### 2. HomeHero のゴールド・EXP がモック固定値
...
| 中 | バトル内アイテムをインベントリと接続 | `BattleCanvas.tsx` |
