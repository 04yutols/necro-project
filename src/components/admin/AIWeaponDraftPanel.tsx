 ...
 * 武器草案パネル。
 *
 * 武器はレアリティが構成（サブオプション枠・パッシブ複雑度）を決めるため、まずレアリティを選ぶ。
 * 決定論的バリデータ（設計書13/92）の結果を表示し、onApply で ItemForm に流し込む。
 */

import { useState } from 'react';
import {
  generateWeaponDraftAction,
  type GenerateWeaponActionResult,
...
const RARITY_HINT: Record<string, string> = {
  R: '通常枠1（属性枠なし）', SR: '通常枠1（属性枠なし）',
  SSR: '通常枠1 + 属性枠1', UR: '通常枠1 + 属性枠1・MYTHIC・固有',
};
const LEVEL_COLOR: Record<string, string> = { PASS: '#4ade80', WARN: '#fbbf24', FAIL: '#f87171' };

export default function AIWeaponDraftPanel({ onApply }: Props) {
  const [open, setOpen] = useState(false);
  const [rarity, setRarity] = useState<string>('SSR');
  const [requirements, setRequirements] = useState('');
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<GenerateWeaponActionResult | null>(null);
  ...
  async function handleGenerate() {
    ...
    setApplied(false);
    try {
      const res = await generateWeaponDraftAction(requirements, rarity);
      setResult(res);
  ...
  return (
    <div style={{ background: 'linear-gradient(180deg, rgba(139,0,255,0.08), rgba(17,17,24,0.6))', border: '1px solid rgba(139,0,255,0.3)', borderRadius: 10, marginBottom: 16, overflow: 'hidden' }}>
      ...
      >
        <span>◈ AI 草案で武器を生成（レアリティに合わせる）</span>
      ...
      {open && (
        <div style={{ padding: '0 16px 16px' }}>
          <p style={{ fontSize: 12, color: '#9090b0', marginBottom: 8 }}>
            武器はレアリティで構成が決まります。レアリティを選ぶと、設計書13/92 のサブオプション枠数・
            パッシブ2スロット構成に沿って生成し、検証します。
          </p>

          <div style={{ marginBottom: 8 }}>
            <label style={{ fontSize: 11, color: '#9090b0', display: 'block', marginBottom: 4 }}>対象レアリティ</label>
              ...
              {RARITIES.map((r) => (
                <button
                  ...
                  style={{
                    ...
                    background: rarity === r ? 'rgba(139,0,255,0.3)' : 'rgba(255,255,255,0.04)',
                    border: `1px solid ${rarity === r ? 'rgba(139,0,255,0.5)' : 'rgba(255,255,255,0.1)'}`,
                    color: rarity === r ? '#e0d0ff' : '#9090b0',
                  }}
                >
                  {r}
                </button>
            ...
            <div style={{ fontSize: 10, color: '#7878a8', marginTop: 4 }}>{RARITY_HINT[rarity]}</div>
          </div>

          <textarea
            value={requirements}
            onChange={(e) => setRequirements(e.target.value)}
            placeholder="例: 闇属性に特化した魔法剣。会心ダメージを伸ばし、魔神化と噛み合うパッシブを持つ。"
            rows={2}
