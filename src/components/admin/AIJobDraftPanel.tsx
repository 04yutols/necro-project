...
/**
 * 職業草案パネル。tier が設計思想を決めるため、まず Tier を選ぶ。
 * 決定論的バリデータ（statModifiers倍率 / skills整合）の結果を表示し、onApply で JobForm へ流し込む。
 * baseStatsByLevel はフォームが補間生成する。
 */

import { useState } from 'react';
import {
  generateJobDraftAction,
  type GenerateJobActionResult,
...
export default function AIJobDraftPanel({ onApply }: Props) {
  const [open, setOpen] = useState(false);
  const [tier, setTier] = useState<number>(2);
  const [requirements, setRequirements] = useState('');
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<GenerateJobActionResult | null>(null);
  ...
  async function handleGenerate() {
    ...
    setApplied(false);
    try {
      const res = await generateJobDraftAction(requirements, tier);
      setResult(res);
  ...
  return (
    <div style={{ background: 'linear-gradient(180deg, rgba(139,0,255,0.08), rgba(17,17,24,0.6))', border: '1px solid rgba(139,0,255,0.3)', borderRadius: 10, marginBottom: 16, overflow: 'hidden' }}>
      ...
      >
        <span>⚔ AI 草案で職業を生成（Tier に合わせる）</span>
      ...
      {open && (
        <div style={{ padding: '0 16px 16px' }}>
          <p style={{ fontSize: 12, color: '#9090b0', marginBottom: 8 }}>
            Tier を選ぶと、設計思想（Tier1=素直な初期職 / Tier2=尖った上位職）に沿って statModifiers・
            スキル習得を生成し、倍率バランスとスキル整合を検証します。基礎ステータス表(lv1〜100)は
            保存後にフォームが補間します。
          </p>

          <div style={{ marginBottom: 8 }}>
            <label style={{ fontSize: 11, color: '#9090b0', display: 'block', marginBottom: 4 }}>Tier</label>
              ...
              {[1, 2].map((t) => (
                <button
                  ...
                  style={{
                    ...
                    background: tier === t ? 'rgba(139,0,255,0.3)' : 'rgba(255,255,255,0.04)',
                    border: `1px solid ${tier === t ? 'rgba(139,0,255,0.5)' : 'rgba(255,255,255,0.1)'}`,
                    color: tier === t ? '#e0d0ff' : '#9090b0',
                  }}
                >
                  Tier {t}{t === 1 ? '（初期職）' : '（上位職）'}
                </button>
          ...
          <textarea
            ...
            placeholder="例: 雷を操る素早い物理アタッカー。会心に特化し、耐久は低い紙装甲。"
            rows={2}
