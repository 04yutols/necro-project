...
/**
 * 素材草案パネル。rarity が expValue 帯を決めるため、まずレアリティを選ぶ。
 * 決定論的バリデータの結果を表示し、onApply で MaterialForm へ流し込む。
 */

import { useState } from 'react';
import {
  generateMaterialDraftAction,
  type GenerateMaterialActionResult,
...
export default function AIMaterialDraftPanel({ onApply }: Props) {
  const [open, setOpen] = useState(false);
  const [rarity, setRarity] = useState<string>('COMMON');
  const [requirements, setRequirements] = useState('');
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<GenerateMaterialActionResult | null>(null);
  ...
  async function handleGenerate() {
    ...
    setApplied(false);
    try {
      const res = await generateMaterialDraftAction(requirements, rarity);
      setResult(res);
  ...
  return (
    <div style={{ background: 'linear-gradient(180deg, rgba(139,0,255,0.08), rgba(17,17,24,0.6))', border: '1px solid rgba(139,0,255,0.3)', borderRadius: 10, marginBottom: 16, overflow: 'hidden' }}>
      ...
      >
        <span>◇ AI 草案で素材を生成（レアリティに合わせる）</span>
      ...
      {open && (
        <div style={{ padding: '0 16px 16px' }}>
          <p style={{ fontSize: 12, color: '#9090b0', marginBottom: 8 }}>
            素材はレアリティで expValue 帯が決まります。レアリティを選ぶと、既存の帯に沿った素材を生成し、
            レアリティ逆転などを検証します。
          </p>

          <div style={{ marginBottom: 8 }}>
            <label style={{ fontSize: 11, color: '#9090b0', display: 'block', marginBottom: 4 }}>レアリティ</label>
          ...
          <textarea
            ...
            placeholder="例: 竜系の敵から得られる希少な強化素材。"
            rows={2}
