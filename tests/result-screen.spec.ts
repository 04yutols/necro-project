...
import { clickBattleAutoOn, prepareE2EPage, setBattleSpeed, startFirstDungeonBattle } from './helpers/e2e';
...
test.describe('Result and appraisal UX', () => {
  ...
  test('reaches result, transitions to appraisal, and exposes reward reveal controls', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /次の戦利品|獲得して戻る|ネクロマンスへ/ })).toBeVisible({
      timeout: 10000,
    });
  });
});
